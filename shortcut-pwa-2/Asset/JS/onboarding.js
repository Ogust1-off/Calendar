    ...
    html: '<div class="ob-hero">' +
      '<div class="ob-logo">' +
        '<div class="ob-logo-icon">' +
          ...
          '</svg>' +
        '</div>' +
        '<div class="ob-logo-wordmark">Shortcut<sup>™</sup></div>' +
      '</div>' +
      '<h1 class="ob-title">Ton agenda<br><em>ECAM</em>, toujours<br>à portée.</h1>' +
...
function obStepDone() {
  return {
    html: '<div class="ob-done">' +
      ...
      '<p class="ob-done-sub">Ton agenda ECAM va se charger dans un instant.</p>' +
    '</div>',
    bottomHTML:
      '<button class="ob-btn ob-btn-primary ob-btn-full" onclick="obFinish()">' +
        'Ouvrir Shortcut <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 3l5 5-5 5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>' +
      '</button>'
  };
}
...
(function injectObCSS() {
  ...
  s.textContent = [
    ...
    '.ob-sheet{position:relative;width:100%;background:#0d1017;border-radius:30px 30px 0 0;padding:0 0 calc(28px + env(safe-area-inset-bottom));overflow:hidden;min-height:72vh;max-height:96vh;display:flex;flex-direction:column;transform:translateY(60px);transition:transform .45s cubic-bezier(.32,1.2,.45,1);box-shadow:0 -20px 80px rgba(0,0,0,.7)}',
    '#ob-overlay.ob-visible .ob-sheet{transform:translateY(0)}',
    '.ob-bg-orbs{position:absolute;inset:0;pointer-events:none;overflow:hidden;border-radius:inherit}',
    ...
    '.ob-content{flex:1;overflow-y:auto;padding:36px 28px 8px;-webkit-overflow-scrolling:touch;position:relative;z-index:1}',
    '.ob-bottom{padding:16px 28px 0;position:relative;z-index:1;flex-shrink:0}',
    ...
    '.ob-input{width:100%;padding:14px 46px 14px 16px;background:rgba(255,255,255,.06);border:1.5px solid rgba(255,255,255,.1);border-radius:14px;color:rgba(255,255,255,.88);font-size:15px;font-family:-apple-system,"SF Pro Text",system-ui,sans-serif;outline:none;transition:border-color .18s,background .18s,box-shadow .18s;-webkit-appearance:none;box-sizing:border-box}',
